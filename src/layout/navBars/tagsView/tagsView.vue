  ...
  setup() {
    ...
    const initTagsView = async () => {
      if (Session.get("tagsViewList") && getThemeConfig.value.isCacheTagsView) {
        ...
        await state.tagsViewRoutesList.map((v: any) => {
          if (v.meta.is_affix && !v.meta.is_hide) {
    ...
    const solveAddTagsView = async (path: string, to?: any) => {
      ...
      if (current.length <= 0) {
        ...
        if (findItem.meta.is_affix) return false;
        if (findItem.meta.is_link && !findItem.meta.is_iframe) return false;
        if (to.meta.isDynamic) findItem.params = to.params;
        else findItem.query = to.query;
    ...
    const singleAddTagsView = (path: string, to?: any) => {
      ...
      state.tagsViewList.forEach((v) => {
        if (
          ...
          if (to.meta.isDynamic) v.params = to.params;
          else v.query = to.query;
          v.url = setTagsViewHighlight(v);
          addBrowserSetSession(state.tagsViewList);
        }
      });
    ...
    const addTagsView = async (path, to) => {
      ...
      if (to && to.meta.isDynamic) {
        ...
        if (state.tagsViewList.some((v) => v.path === to.meta.isDynamicPath)) return;
        item = state.tagsViewRoutesList.find((v) => v.path === to.meta.isDynamicPath);
      } else {
        if (!getThemeConfig.value.isShareTagsView) await solveAddTagsView(path, to);
        else await singleAddTagsView(path, to);
        if (state.tagsViewList.some((v) => v.path === path)) return;
        item = state.tagsViewRoutesList.find((v) => v.path === path);
      }
      if (!item) return;
      if (item.meta.is_link && !item.meta.is_iframe) return;
      if (to && to.meta.isDynamic) item.params = to?.params || route.params;
      else item.query = to?.query || route.query;
    ...
    const refreshCurrentTagsView = async (fullPath: string) => {
      const item = state.tagsViewList.find((v: any) =>
        getThemeConfig.value.isShareTagsView ? v.path === fullPath : v.url === fullPath,
      );
      if (item != null) {
        await storesKeepALiveNames.delCachedView(item);
        proxy.mittBus.emit("onTagsViewRefreshRouterView", fullPath);
        if (item.meta.is_keep_alive) storesKeepALiveNames.addCachedView(item);
      }
    };
    ...
    const getCurrentRouteItem = (path: string, cParams: any) => {
      const itemRoute = Session.get("tagsViewList")
    ...
    const initSortable = async () => {
      ...
      if (!el) return false;
      if (state.sortable.el) state.sortable.destroy();
    ...
    onBeforeMount(() => {
      ...
      onSortableResize();
      // 拖动问题，加防抖避免高频触发
      window.addEventListener("resize", debounceSortableResize);
      // 监听非本页面调用 0 刷新当前，1 关闭当前，2 关闭其它，3 关闭全部 4 当前页全屏
      proxy.mittBus.on("onCurrentContextmenuClick", onContextmenuClick);
    });
    onUnmounted(() => {
      // 取消非本页面调用监听（使用命名函数引用，确保正确移除）
      proxy.mittBus.off("onCurrentContextmenuClick", onContextmenuClick);
      // 取消监听布局配置界面开启/关闭拖拽
      proxy.mittBus.off("openOrCloseSortable", onOpenOrCloseSortable);
      // 取消监听布局配置开启 TagsView 共用
      proxy.mittBus.off("openShareTagsView", onOpenShareTagsView);
      // 取消窗口 resize 监听
      window.removeEventListener("resize", debounceSortableResize);
    });
    // 页面更新时
    onBeforeUpdate(() => {
      tagsRefs.value = [];
    ...
    // 仅监听 tagsViewRoutes 长度变化，而非整个 pinia.state（性能优化）
    watch(
...
.layout-navbars-tagsview {
  background-color: var(--cc-color-bg-elevated);
...
.layout-navbars-tagsview-ul-li:hover {
  background-color: var(--cc-color-primary-softer);
  color: var(--cc-color-primary);
  border-color: color-mix(in srgb, var(--cc-color-primary) 50%, transparent);
}
...
.layout-navbars-tagsview-ul .layout-icon-active {
  display: block;
}
.layout-navbars-tagsview-ul .layout-icon-three {
  display: none;
}
...
/* tags-style-four */
.tags-style-four .layout-navbars-tagsview-ul-li {
  margin-right: 0 !important;
  border: none !important;
  position: relative;
  border-radius: var(--cc-radius-sm) !important;
}
.tags-style-four .layout-navbars-tagsview-ul-li .layout-icon-active {
  display: none;
}
.tags-style-four .layout-navbars-tagsview-ul-li .layout-icon-three {
  display: block;
}
.tags-style-four .layout-navbars-tagsview-ul-li:hover {
  background: none !important;
}
.tags-style-four .is-active {
  background: none !important;
  color: var(--cc-color-primary) !important;
}

/* tags-style-five （梯形 mask 形状） */
.tags-style-five {
...
.tags-style-five .layout-navbars-tagsview-ul-li {
  padding: 0 5px;
...
.tags-style-five .layout-navbars-tagsview-ul-li .layout-icon-active,
.tags-style-five .layout-navbars-tagsview-ul-li .layout-navbars-tagsview-ul-li-iconfont,
.tags-style-five .layout-navbars-tagsview-ul-li .layout-navbars-tagsview-ul-li-refresh {
  display: none;
}
.tags-style-five .layout-navbars-tagsview-ul-li .layout-icon-three {
  display: block;
}
.tags-style-five .layout-navbars-tagsview-ul-li:hover,
.tags-style-five .is-active {
  -webkit-mask-box-image: url("data:image/svg+xml,%3Csvg width='68' height='34' viewBox='0 0 68 34' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath fill-rule='evenodd' clip-rule='evenodd' d='m27,0c-7.99582,0 -11.95105,0.00205 -12,12l0,6c0,8.284 -0.48549,16.49691 -8.76949,16.49691l54.37857,-0.11145c-8.284,0 -8.60908,-8.10146 -8.60908,-16.38546l0,-6c0.11145,-12.08445 -4.38441,-12 -12,-12l-13,0z' fill='%23409eff'/%3E%3C/svg%3E")
    12 27 15;
}
.tags-style-five .layout-navbars-tagsview-ul-li:hover {
...
.tags-style-five .is-active {
  background: var(--cc-color-primary-softer) !important;
  color: var(--cc-color-primary) !important;
  z-index: 1;
}
