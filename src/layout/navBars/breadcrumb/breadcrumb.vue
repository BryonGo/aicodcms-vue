    ...
    const initRouteSplit = (path: string) => {
      ...
      if (state.breadcrumbList.length > 0)
        state.breadcrumbList[state.breadcrumbList.length - 1].meta.tagsViewName =
          other.setTagsViewNameI18n(route);
    };
    // 页面加载时
    onMounted(() => {
      initRouteSplit(route.path);
...
.layout-navbars-breadcrumb-icon:hover {
  opacity: 1;
...
.layout-navbars-breadcrumb :deep(.el-breadcrumb__inner a),
.layout-navbars-breadcrumb :deep(.el-breadcrumb__inner.is-link) {
  font-weight: 500 !important;
  color: var(--cc-color-text-3) !important;
  transition: color 0.18s ease;
}
.layout-navbars-breadcrumb :deep(.el-breadcrumb__inner a:hover),
.layout-navbars-breadcrumb :deep(.el-breadcrumb__inner.is-link:hover) {
