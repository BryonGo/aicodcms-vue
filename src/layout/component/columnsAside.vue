      ...
      <ul @mouseleave="onColumnsAsideMenuMouseleave()">
        ...
        >
          <div
            ...
            v-if="!v.meta.is_link || (v.meta.is_link && v.meta.is_iframe)"
          >
          ...
          <div :class="themeConfig.columnsAsideLayout" v-else>
            <a :href="v.meta.is_link" target="_blank">
              <SvgIcon :name="v.meta.icon" />
              <div class="columns-vertical-title font12">
                {{
                  $t(v.meta.title) && $t(v.meta.title).length >= 4
...
export default defineComponent({
  ...
  setup() {
    ...
    const filterRoutesFun = (arr: Array<string>) => {
      return arr
        .filter((item: any) => !item.meta.is_hide)
        .map((item: any) => {
    ...
    watch(
      pinia.state,
      (val) => {
        state.difference = val.themeConfig.themeConfig.columnsAsideStyle === "columnsRound" ? 3 : 0;
...
.layout-columns-aside ul {
  position: relative;
}
.layout-columns-aside ul li {
...
.layout-columns-aside ul li .columns-vertical {
  margin: auto;
}
.layout-columns-aside ul li .columns-vertical .columns-vertical-title {
  padding-top: 1px;
}
.layout-columns-aside ul li .columns-horizontal {
  display: flex;
  height: 50px;
  width: 100%;
  align-items: center;
...
.layout-columns-aside ul li .columns-horizontal i {
  margin-right: 3px;
}
.layout-columns-aside ul li .columns-horizontal a {
  display: flex;
}
.layout-columns-aside ul li .columns-horizontal a .columns-horizontal-title {
  padding-top: 1px;
}
.layout-columns-aside ul li a {
  text-decoration: none;
  color: var(--cc-color-text-2);
}

.layout-columns-aside ul .layout-columns-active {
  color: var(--cc-color-text-inverse) !important;
  transition: color 0.3s ease-in-out;
}
.layout-columns-aside ul .layout-columns-hover {
  color: var(--cc-color-primary);
}
.layout-columns-aside ul .layout-columns-hover a {
  color: var(--cc-color-primary);
}

.layout-columns-aside ul .columns-round {
...
.layout-columns-aside ul .columns-card {
  background: var(--cc-color-primary);
  color: var(--cc-color-text-inverse);
  position: absolute;
  left: 50%;
