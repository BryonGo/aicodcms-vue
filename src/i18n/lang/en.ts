export default {
  router: {
    home: "Home",
    loading: "Loading",
    login: "Login",
    system: "System",
    systemMenu: "System Menu",
    cmsTplTagGenerator: "TAG Management",
    cmsChannelAdd: "Add Channel",
    cmsChannelEdit: "Edit Channel",
    cmsArticleEdit: "Edit Article",
    cmsBlockAdd: "Add Block",
    cmsBlock: "Blocks",
    cmsBlockEdit: "Edit Block",
    cmsPageAdd: "Add Page",
    cmsPageEdit: "Edit Page",
    cmsTagAdd: "Add Tag",
    cmsTagEdit: "Edit Tag",
    cmsLinkAdd: "Add Link",
    cmsLinkEdit: "Edit Link",
    pmsModulesInfoAdd: "Add Model",
    pmsModulesInfoEdit: "Edit Model",
    pmsModulesInfoGenModelConfirm:
      "Generating the model will back up and delete original table data. Continue?",
    pmsModulesFieldAdd: "Add Field",
    pmsModulesFieldEdit: "Edit Field",
    pmsDictAdd: "Add Dict",
    pmsDictEdit: "Edit Dict",
    pmsDictDataAdd: "Add Dict Data",
    pmsDictDataEdit: "Edit Dict Data",
    pmsDeptAdd: "Add Dept",
    pmsDeptEdit: "Edit Dept",
    pmsPostAdd: "Add Post",
    pmsPostEdit: "Edit Post",
    pmsParamAdd: "Add Param",
    pmsParamEdit: "Edit Param",
    pmsUserAdd: "Add User",
    pmsUserEdit: "Edit User",
    pmsRoleAdd: "Add Role",
    pmsRoleEdit: "Edit Role",
    pmsMenuAdd: "Add Menu",
    pmsMenuEdit: "Edit Menu",
    addonJobAdd: "Add Job",
    addonJobEdit: "Edit Job",
    addonShortlink: "Short Links",
    addonShortio: "Short.io Links",

    cmsArticleAdd: "Add Article",
  },
  statusDraft: "Draft",
  statusDraftHint: "Please enter a title before saving as draft",
  statusTitleRequired: "Title is required",
  staticRoutes: {
    notFound: "notFound",
    noPower: "noPower",
  },
  user: {
    title0: "Component size",
    title1: "Language switching",
    title2: "Menu search",
    title3: "Layout configuration",
    title5: "Full screen on",
    title6: "Full screen off",
    dropdownLarge: "large",
    dropdownDefault: "default",
    dropdownSmall: "small",
    dropdown1: "home page",
    dropdown3: "404",
    dropdown4: "401",
    dropdown5: "Log out",
    searchPlaceholder: "Menu search: support Chinese, routing path",
    newTitle: "notice",
    newBtn: "All read",
    newGo: "Go to the notification center",
    newDesc: "No notice",
    logOutTitle: "Tips",
    logOutMessage: "This operation will log out. Do you want to continue?",
    logOutConfirm: "determine",
    logOutCancel: "cancel",
    logOutExit: "Exiting",
    langSwitchSuccess: "Language switched successfully, page will reload",
    noPower: "No permission",
    cmsArticleAdd: "Add Article",
    cmsBlock: "Blocks",
    cmsBlockEdit: "Edit Block",
    loading: "Loading",
    versionConsole: "Console",
    versionApi: "API",
    versionUnset: "not set",
    versionDriftHint: "⚠ Version mismatch — the deploy may be half-done",
    versionFailedHint: "⚠ API version unavailable — cannot tell if the deploy is half-done",
    versionUnknown: "unknown",
    versionTipConsole:
      "Console version comes from version.json written at container start (= image tag); unavailable here, usually a local dev environment",
    versionTipApi: "API version comes from /api/version (the Go process currently running)",
    versionTipApiFailed:
      "API version unavailable: the /api/version request failed (this environment may not run an API with that endpoint yet)",
  },
  tagsView: {
    refresh: "refresh",
    close: "close",
    closeOther: "closeOther",
    closeAll: "closeAll",
    fullscreen: "fullscreen",
    closeFullscreen: "closeFullscreen",
  },
  notFound: {
    foundTitle: "Wrong address input, please re-enter the address~",
    foundMsg:
      "You can check the web address first, and then re-enter or give us feedback.",
    foundBtn: "Back to home page",
  },
  noAccess: {
    accessTitle: "You are not authorized to operate~",
    accessMsg:
      "This module may not be installed or you don't have permission. Contact the administrator.",
    accessBtn: "Reauthorization",
  },
  layout: {
    configTitle: "Layout configuration",
    oneTitle: "Global Themes",
    twoTopTitle: "top bar set up",
    twoMenuTitle: "Menu set up",
    twoColumnsTitle: "Columns set up",
    twoTopBar: "Top bar background",
    twoTopBarColor: "Top bar default font color",
    twoIsTopBarColorGradual: "Top bar gradient",
    twoMenuBar: "Menu background",
    twoMenuBarColor: "Menu default font color",
    twoIsMenuBarColorGradual: "Menu gradient",
    twoColumnsMenuBar: "Column menu background",
    twoColumnsMenuBarColor: "Default font color bar menu",
    twoIsColumnsMenuBarColorGradual: "Column gradient",
    threeTitle: "Interface settings",
    threeIsCollapse: "Menu horizontal collapse",
    threeIsUniqueOpened: "Menu accordion",
    threeIsFixedHeader: "Fixed header",
    threeIsClassicSplitMenu: "Classic layout split menu",
    threeIsLockScreen: "Open the lock screen",
    threeLockScreenTime: "screen locking(s/s)",
    fourTitle: "Interface display",
    fourIsShowLogo: "Sidebar logo",
    fourIsBreadcrumb: "Open breadcrumb",
    fourIsBreadcrumbIcon: "Open breadcrumb icon",
    fourIsTagsview: "Open tagsview",
    fourIsTagsviewIcon: "Open tagsview Icon",
    fourIsCacheTagsView: "Enable tagsview cache",
    fourIsSortableTagsView: "Enable tagsview drag",
    fourIsShareTagsView: "Enable tagsview sharing",
    fourIsFooter: "Open footer",
    fourIsGrayscale: "Grey model",
    fourIsInvert: "Color weak mode",
    fourIsDark: "Dark Mode",
    fourIsWartermark: "Turn on watermark",
    fourWartermarkText: "Watermark copy",
    fiveTitle: "Other settings",
    fiveTagsStyle: "Tagsview style",
    fiveAnimation: "page animation",
    fiveColumnsAsideStyle: "Column style",
    fiveColumnsAsideLayout: "Column layout",
    sixTitle: "Layout switch",
    sixDefaults: "One",
    sixClassic: "Two",
    sixTransverse: "Three",
    sixColumns: "Four",
    tipText:
      "Click the button below to copy the layout configuration to `/src/stores/themeConfig.ts` It has been modified in.",
    copyText: "replication configuration",
    resetText: "restore default",
    copyTextSuccess: "Copy succeeded!",
    copyTextError: "Copy failed!",
  },

  // ==================== Global Messages ====================
  msg: {
    loadFailed: "Load failed",
    error: "Error",
    warning: "Warning",
    addSuccess: "Added",
    editSuccess: "Updated",
    deleteSuccess: "Deleted",
    saveSuccess: "Saved",
    createSuccess: "Created",
    weightUpdated: "Weight updated",
  },

  // ==================== Version update bar ====================
  updateBar: {
    title: "A new console version is available",
    autoIn: "Auto refresh in {n}s",
    paused: "Editing in progress, will refresh after you finish",
    refreshNow: "Refresh now",
    later: "Later",
    laterTip: "Do not auto refresh this session; you can refresh anytime",
  },

  common: {
    save: "Save",
    cancel: "Cancel",
    dept: "Dept",
    hidden: "Hidden",
    query: "Query",
    success: "Success",
    close: "Close",
    edit: "Edit",
    delete: "Delete",
    add: "Add",
    search: "Search",
    reset: "Reset",
    status: "Status",
    yes: "Yes",
    noData: "No data",
    type: "Type",
    all: "All",
    back: "Back",
    backToList: "Back to List",
    submit: "Submit",
    no: "No",
    disabled: "Disabled",
    enabled: "Enabled",
    normal: "Normal",
    detail: "Detail",
    confirm: "Confirm",
    refresh: "Refresh",
    create: "Create",
    settings: "Settings",
    expand: "Expand",
    collapse: "Collapse",
    loading: "Loading...",
    saving: "Saving...",
    submitFailed: "Submit Failed",
    msgAddOk: "Added",
    msgEditOk: "Updated",
    msgDeleteOk: "Deleted",
    msgSaveOk: "Saved",
    msgSortOk: "Sorted",
    msgSortFail: "Sort Failed",
    msgNetworkError: "Network Error",
    msgNetworkErrorRetry: "Network error, please retry",
    msgUnknown: "Unknown",
    btnDelete: "Delete",
    btnDetail: "Detail",
    btnRemove: "Delete",
    btnAdd: "Add",
    btnBack: "Back",
    btnCancel: "Cancel",
    btnSave: "Save",
    confirmDeleteBatch: "Delete selected data?",
    confirmDeleteSingle: "Permanently delete this data?",
    confirmTitle: "Confirm",
    msgSelectDelete: "Please select data to delete.",
    colOperation: "Actions",
    colCreateTime: "Created",
    colUpdateTime: "Updated",
    colId: "ID",
    colStatus: "Status",
    colSort: "Sort",
    colName: "Name",
    colUrl: "URL",
    colRemark: "Remark",
    msgNameRequired: "Name is required",
    msgTableNameRequired: "Table name is required",
    requiredSuffix: " is required",
    confirmDeleteItem: 'Delete "{name}"? This cannot be undone.',
    second: "s",
    minute: "min",
    hour: "h",
    day: "d",
    preview: "Preview",
    default: "Default",
    zhCn: "Simplified Chinese",
    progress: "Progress",
    confirmDeleteTitle: "Confirm Delete",
    confirmEdit: "Edit",
    confirmAdd: "Add",
    statusShow: "Show",
    gender: {
      male: "Male",
      female: "Female",
      secrecy: "Private",
    },
    statusHide: "Hide",
    btnEdit: "Edit",
    btnSearch: "Search",
    btnReset: "Reset",

    description: "Description",
    enter: "Enter",
    export: "Export",
    failure: "Failure",
    import: "Import",
    msgSuccess: "Operation successful",
    operation: "Operation",
    remark: "Remark",
    selAll: "Select All",
    switch: "Switch",
    tips: "Tips",
    updateTime: "Updated At",
    weight: "Weight",

    saveSuccess: "Saved",
    deleteSuccess: "Deleted",
    pressEscToClose: "Press Esc to close",
    batchDelete: "Batch Delete",
  },
  // 模型字段翻译（扁平 key: moduleField_{name} / moduleFieldOpt_{name}_{optKey}）
  moduleField_language: "Language",
  moduleField_region: "Region",
  moduleField_plot: "Plot",
  moduleField_director: "Director",
  moduleField_duration: "Duration",
  moduleField_year: "Year",
  "moduleFieldOpt_language_zh-cn": "Chinese",
  "moduleFieldOpt_language_en-us": "English",
  moduleFieldOpt_region_mainland: "Mainland China",
  moduleFieldOpt_region_hktw: "HK & Taiwan",
  moduleFieldOpt_region_euus: "Europe & US",
  moduleFieldOpt_region_jpkr: "Japan & Korea",
  moduleFieldOpt_region_other: "Other",
  sdk: {
    gameList: "SDK Game Management",
    gameAdd: "Add Game",
    gameEdit: "Edit Game",
    productList: "SDK Product Management",
    productAdd: "Add Product",
    productEdit: "Edit Product",
    versionList: "SDK Version Management",
    versionAdd: "Add Version",
    versionEdit: "Edit Version",
    game: {
      breadcrumbHome: "Home",
      breadcrumbSdk: "SDK",
    },
    version: {
      breadcrumbVersion: "Versions",
      addSubtitle: "Create a new game version",
      addSectionInfo: "Version Info",
      addLabelVersionCode: "Version Code",
      addLabelVersionName: "Version Name",
      addLabelVersionDesc: "Release Notes",
      addPlaceholderVersionDesc: "Enter release notes...",
      addLabelUrl: "Download URL",
      addPlaceholderUrl: "APK/IPA download URL",
      addLabelForceUpdate: "Force Update",
      addLabelEnabled: "Enabled",
      addBtnSubmitting: "Submitting...",
      yes: "Yes",
      no: "No",
      enabled: "Enabled",
      disabled: "Disabled",
      editLoading: "Loading version info...",
      editErrorTitle: "Failed to load version info",
      editSectionInfo: "Version Info",
      editLabelVersionCode: "Version Code",
      editLabelVersionName: "Version Name",
      editLabelVersionDesc: "Release Notes",
      editPlaceholderDesc: "Enter release notes...",
      editLabelUrl: "Download URL",
      editLabelForceUpdate: "Force Update",
      editLabelEnabled: "Enabled",
      saveBtnText: "Save Changes",
      codeLabel: "Version Code",
      activeTag: "Active",
      inactiveTag: "Inactive",
      metaCreated: "Created",
      metaUpdated: "Updated",
      msgVersionCodeRequired: "Version code is required",
      addSuccess: "Version added successfully",
      saveSuccess: "Saved successfully",
      deleted: "Deleted",
      deleteConfirm: "Are you sure to delete version {{code}}?",
      deleteTitle: "Delete Version",
      batchDelete: "Batch Delete ({{count}})",
      batchDeleteConfirm: "Are you sure to delete {{count}} selected versions?",
      batchDeleteOk: "Batch delete successful",
      btnDeactivate: "Deactivate",
      btnActivate: "Activate",
      toggleActiveTitle: "Toggle Version Status",
      toggleActiveConfirm: "Are you sure to {{action}} version {{code}}?",
      toggleActiveOk: "Status updated",
      editMissingId: "Missing version ID",
      editErrorNotFound: "Version not found",
      editErrorNetwork: "Network error, please retry",
      searchPlaceholder: "Search by version name",
      gameLabel: "Game「{{name}}」(ID: {{id}})",
      backToGame: "Back to games",
      headerTitle: "Version Management",
      unnamed: "Unnamed",
      forceYes: "Force",
      forceNo: "Optional",
      activeYes: "Active",
      activeNo: "Inactive",
      colVersionCode: "Version Code",
      colVersionName: "Version Name",
      colVersionDesc: "Description",
      colForceUpdate: "Force Update",
    },
    payConfigList: "SDK Payment Config",
    payConfigAdd: "Add Payment Config",
    payConfigEdit: "Edit Payment Config",
    developerList: "SDK Developer Management",
    developerAdd: "Add Developer",
    developerEdit: "Edit Developer",
    orderList: "SDK Order Management",
    orderDetail: "Order Detail",
    completedList: "SDK Dedup",
    gameOrder: "Game Orders",
    gameUserList: "Game User",
    gameUserRole: "Role Binding",
    refundList: "SDK Refund Records",
    siteList: "Site Management",
    siteAdminList: "Site Admins",
    sitelinkList: "Site Links",
    notifyLogList: "SDK Notify Log",
    banList: "SDK Ban Management",
    banAdd: "Add Ban",
    banEdit: "Edit Ban",
    loginLogList: "SDK Login Log",
    userList: "SDK User",
    userDetail: "User Detail",
    payChannelList: "SDK Pay Channel",
    payChannelAdd: "Add Pay Channel",
    payChannelEdit: "Edit Pay Channel",
    currencyList: "SDK Currency",
    currencyAdd: "Add Currency",
    currencyEdit: "Edit Currency",
    email: "Email Service",
    platformTasks: "Task Overview",
    platformGroup: "Platform Ops",
    platformModelSwitch: "Model Switches",
    platformVideoOption: "Video Options",
    platformLora: "LoRA Catalog",
    platformUsers: "Users & Credits",
    platformTags: "Tags",
    platformHougongTools: "Creation Tools",
    platformLedger: "Ledger",
    platformProviders: "Provider Health",
    platformRates: "Billing Rates",
    platformPrompts: "Prompt Library",
    platform: {
      aiProviderTitle: "AI Providers & Models",
      canvasInspectTitle: "Canvas Inspector",
      aiProviderSubtitle: "Manage upstream providers (protocol, base_url, credential) and models (display name decoupled from upstream name). Adding a provider only requires picking a protocol.",
      btnNewProvider: "New Provider",
      btnNewModel: "New Model",
      btnProbe: "Test",
      btnSync: "Sync Models",
      btnEdit: "Edit",
      btnDelete: "Delete",
      btnSave: "Save",
      btnCancel: "Cancel",
      btnQuery: "Refresh",
      emptyProviders: "No AI provider configured for this site yet. Click \"New Provider\" to pick a protocol and fill in base_url and credential.",
      colName: "Name",
      colProtocol: "Protocol",
      colBaseUrl: "Base URL",
      colKey: "Credential",
      keySet: "Set",
      keyUnset: "Unset",
      colEnabled: "Enabled",
      on: "On",
      off: "Off",
      colProbe: "Last Probe",
      probeNever: "Never",
      probeOk: "Connection OK",
      probeFail: "Connection failed",
      colActions: "Actions",
      colModelListPath: "Model List Path",
      colTimeout: "Timeout (ms)",
      colRemark: "Remark",
      modelsOf: "Models ·",
      selectProviderHint: "Select a provider above",
      noModels: "No model under this provider yet. Use \"Sync Models\" or add manually.",
      normColFamily: "Family",
      normColVersion: "Version",
      normColIsDefault: "Live version",
      colModelRemark: "Remark (admin only)",
      phModelRemark: "e.g. safe content only — swimwear/adult gets blocked upstream",
      hintModelRemark: "Admin-only note: upstream content limits, whether adult content works here, etc. Never shown to users.",
      normFamilyNone: "Not normalized",
      normPhFamily: "e.g. Seedream 5 (blank = its own entry)",
      normHintFamily: "This is the name users see. All versions in one family collapse into a single entry; which version ships is decided by \"Live version\".",
      normPhVersion: "e.g. 5.0 Pro / 4.5",
      normHintVersion: "For admin and troubleshooting only; never sent to clients.",
      normHintIsDefault: "When on, this family ships this version; other rows in the family are unset automatically.",
      colDisplayName: "Display Name",
      colUpstream: "Upstream Model",
      colKind: "Kind",
      colState: "State",
      stateAvailable: "Available",
      stateHidden: "Hidden",
      colModelId: "Internal ID",
      colOutputFormat: "Output Format",
      colAuthor: "Author",
      colExcerpt: "Excerpt",
      colSort: "Sort",
      colBilling: "Billing JSON",
      missingTag: "Missing upstream",
      autoHiddenTag: "Auto disabled",
      configGapTag: "Incomplete",
      configGapHint:
        "Capabilities/billing are incomplete, so this model is hidden from users (selectable but not orderable is worse than absent). Hover the tag for the exact gap; it appears automatically once filled.",
      btnOrder: "Reorder",
      orderTitle: "Reorder front-end entries",
      orderKindImage: "Image models",
      orderKindVideo: "Video models",
      orderHint:
        "Ordering is per front-end entry: versions of one family collapse into one entry, so each row here is one entry users actually see. The server renumbers — no need to type sort numbers. Models disabled by the model switch stay listed here so they keep their position for when they are re-enabled.",
      orderColEntry: "Entry",
      orderColKey: "Key (family name / model id)",
      orderVersions: "{n} versions",
      orderLoadFailed: "Failed to load order",
      orderMoveFailed: "Failed to reorder",
      phProviderName: "e.g. Xiaoyi",
      phProtocol: "Pick a protocol",
      hintProtocol: "The protocol determines the call shape, independent of the provider name.",
      hintBaseUrl: "Upstream base URL, the single source of truth. A trailing /vN is de-duplicated against the protocol version.",
      phKey: "Paste upstream API Key",
      phKeyKeep: "Leave blank to keep the saved credential",
      hintKey: "Credential is encrypted at rest and never echoed back. Blank or all-* means unchanged.",
      hintModelListPath: "Path relative to base_url including its version segment (e.g. /v1/models). Blank uses the protocol default.",
      phDisplayName: "e.g. Seedream 5.0 Pro",
      hintDisplayName: "Display name is for the UI only; upstream renames do not affect it.",
      phUpstream: "e.g. dola-seedream-5-0-pro-260628 or ep-...",
      hintUpstream: "The real upstream model name / endpoint id, never exposed to clients.",
      phModelId: "Blank uses the upstream name",
      hintModelId: "The modelId sent to clients; avoid changing after creation.",
      hintJson: "Must be a valid JSON object; blank means unset.",
      jsonInvalid: "Invalid JSON in capabilities/billing",
      saved: "Saved",
      saveFailed: "Save failed",
      deleted: "Deleted",
      deleteFailed: "Delete failed",
      confirmDeleteProvider: "Delete provider \"{name}\"? It will be refused if models still belong to it.",
      confirmDeleteModel: "Delete model \"{name}\"? To take it offline, set state=hidden instead.",
      syncDone: "Sync done: {inserted} added, {existing} existing, {missing} missing upstream",
      syncFailed: "Sync failed (protocol may not support discovery, or upstream error)",
      pullTitle: "Pull Upstream Models",
      pullHint:
        "Fetches the upstream list but writes nothing yet — only the models you tick get written. Already-added models are marked; re-writing them only refreshes the sync time and never overwrites your display name or billing.",
      pullSearchPh: "Search display name or upstream model id",
      pullColUpstream: "Upstream Model",
      pullColLabel: "Upstream Label",
      pullColContext: "Context Window",
      pullColOutput: "Max Output",
      pullExistsTag: "added",
      pullSelectFiltered: "Select Filtered",
      pullClearSelection: "Clear Selection",
      pullWriteSelected: "Write Selected",
      pullSelectedCount: "{n} selected",
      pullNoneSelected: "Tick at least one model to write",
      pullDone: "Written: {inserted} added, {existing} existing",
      pullFailed: "Pull failed (protocol may not support discovery, or upstream error)",
      pullEmpty: "Upstream returned no models (use Full Sync to see the actual error)",
      pullFullSync: "Full Sync (marks missing)",
      pullFullSyncHint:
        "No selection: pulls and writes everything, and marks models the upstream no longer returns as missing — those get auto-disabled after the grace period. Only use it when you are sure the upstream list is complete; otherwise use Write Selected.",
      colContextWindow: "Context Window (tokens)",
      colMaxOutput: "Max Output (tokens)",
      colVision: "Sees images (multimodal)",
      hintVision:
        "Chat models only. When enabled, images wired into the canvas are sent to this model. When disabled the platform silently degrades them to a text description (fail-closed) — no error, just worse output.",
      hintContextWindow:
        "How many tokens of context the model accepts. Filled automatically when pulled (left blank when the upstream does not disclose it).",
      hintMaxOutput: "Maximum tokens in one answer. Also only filled from the pull; blank when undisclosed.",
      secAdvancedCaps: "Other Capabilities JSON",
      colReasoningEfforts: "Reasoning Efforts",
      colReasoningDefault: "Default Reasoning Effort",
      phReasoningEfforts: "Type an effort and press Enter, e.g. low / high / max",
      hintReasoning:
        "Text models only. Effort names are sent to the upstream verbatim (vendors differ: DeepSeek uses off/low/high/max, OpenAI-style uses minimal/low/medium/high), so there are no presets. Blank means the model does not support reasoning effort and the parameter is omitted.",
      reasoningDefaultNone: "Unset (use the upstream default)",
      hintChatCaps:
        "These fields exist for text models (kind=chat) only. Setting them on an image/video model is rejected by the backend — the key would never be read, a silent misconfiguration.",
      revisionConflict:
        "This configuration was changed elsewhere while you were editing (another tab?): the latest values have been reloaded for you. Please review and save again.",
      keyFormatBad:
        "The credential must be printable ASCII and must not be a whole NAME=value line (a common paste from .env files or docs).",
      orderKindChat: "Text Models",
      colToolCover: "Cover image",
      colToolCoverBefore: "Before image",
      colToolCoverVideo: "Card preview video",
      colToolBadge: "Badge",
      colToolTags: "Tags",
      toolsEmptyOnSite:
        "No creation tools on the current site \"{site}\". Tools are isolated per site (the Hougong tools live on the hougong site) — switch the site in the top-right corner.",
      toolsEmptyNoSite:
        "No creation tools yet. Tools are isolated per site, so pick a site in the top-right corner first.",
      colIsCard: "Own card",
      phToolCover: "hougong/media/image/xxx.webp or https://…",
      phToolCoverTpl: "empty = use the parent tool's cover",
      phToolCoverBefore: "The pre-process original. Paired with the cover -> the front end shows a compare slider",
      phToolCoverBeforeTpl: "empty = use the parent tool's before image",
      phToolCoverVideo: "hougong/media/video/xxx.mp4 or https://…",
      hintToolCoverVideo:
        "With a video here, the front-end effect card plays it muted and looping once it enters the viewport (the cover image acts as its poster). Empty keeps the static image.",
      phToolBadge: "Hot / New / Featured (empty = hidden)",
      phToolTags: "undress,full,upper-body,lower-body (comma separated; the effects list filters by these)",
      phToolTagsTpl: "blowjob,deep-throat (comma separated; the effects list filters by these)",
      isCardHint:
        "On: gets its own card on All Effects (blowjob / deepthroat / spread-eagle really are different effects). Off: only appears as an option of its parent tool (full / top / bottom are three choices of one action; separate cards make the list look like duplicates).",
      colToolInput: "Input form",
      colCategory: "Category",
      breadcrumbHome: "Home",
      hougongToolTitle: "Creation Tools",
      hougongToolSubtitle:
        "Hougong's own creation tools and templates, independent of Model Switch: a tool is a capability, a template is a preset under it. Disabling a tool also disables its templates, and the frontend entry disappears immediately.",
      btnCreateTool: "New tool",
      btnEditTool: "Edit tool",
      btnTemplates: "Templates",
      btnCreateTemplate: "New template",
      btnEditTemplate: "Edit template",
      btnBatchEnable: "Enable selected",
      btnBatchDisable: "Disable selected",
      colToolCode: "Code",
      colToolName: "Name",
      colTemplateCount: "Templates",
      colSwitch: "Switch",
      colSummary: "Summary",
      colPromptPreset: "Prompt preset",
      colNegativePreset: "Negative preset",
      colLoraCodes: "Preset LoRAs",
      colParams: "Preset params",
      filterCategory: "Category",
      categoryAll: "All categories",
      promptPresetHint: "Server-side preset; never sent to the client",
      loraCodesHint: "One LoRA filename per line (platform_model.file_name)",
      codeNameRequired: "Code and name are required",
      deleteToolConfirm: 'Delete tool "{name}"? Its templates will be deleted too.',
      deleteTemplateConfirm: 'Delete template "{name}"?',
      breadcrumbSdk: "Platform",
      tasksTitle: "Task Overview",
      tasksSubtitle: "Inspect task status, progress and credit consumption",
      ledgerTitle: "Ledger",
      ledgerSubtitle:
        "Inspect credit ledger entries (grant/reserve/capture/refund)",
      providersTitle: "Provider Health",
      providersSubtitle: "Check ComfyUI provider online status per site",
      ratesTitle: "Billing Rates",
      ratesSubtitle:
        "Inspect credit pricing and provider cost per site/product",
      colId: "ID",
      colSiteId: "Site",
      colSiteCode: "Site Code",
      colAccountId: "Account",
      colProduct: "Product",
      colType: "Type",
      colStatus: "Status",
      colProgress: "Progress",
      colErrorCode: "Error",
      colReservedCredits: "Reserved",
      colBilledCredits: "Billed",
      colCreatedAt: "Created At",
      colOp: "Op",
      colAmount: "Amount",
      colBalanceAfter: "Balance After",
      colTaskId: "Task ID",
      colOrderId: "Order ID",
      colProvider: "Provider",
      colDetail: "Detail",
      colDimKey: "Dim Key",
      colCredits: "Credits",
      colCostCents: "Cost (cents)",
      colRevision: "Revision",
      enabled: "Enabled",
      disabled: "Disabled",
      filterKeyword: "Keyword",
      filterSite: "Site ID",
      filterStatus: "Status",
      filterProduct: "Product",
      filterAccount: "Account ID",
      btnReset: "Reset",
      noData: "No Data",
      statusAll: "All Status",
      productAll: "All Products",
      modelSwitchTitle: "Model Switches",
      modelSwitchSubtitle:
        "Enable/disable base models, LoRAs, video and cloud models; disabled ones disappear from the product and are rejected at task creation",
      videoOptionTitle: "Video Options",
      videoOptionSubtitle:
        "Adult-LoRA switch plus current i2v workflow parameters (read-only, for cross-checking against the reference workflow)",
      loraTitle: "LoRA Catalog",
      loraSubtitle:
        "Import LoRAs from the site ComfyUI and mark them via the NSFW blacklist or by hand",
      usersTitle: "Users & Credits",
      usersSubtitle:
        "User list and detail, credit/balance adjustment (audited), wallet ledger",
      tagsTitle: "Tags",
      tagsSubtitle: "Create and maintain community tags",
      btnAddRate: "Add / Reprice",
      btnBatchDuration: "Batch duration rates",
      rateDialogTitle: "Write Rate",
      rateBatchTitle: "Batch Duration Rates",
      rateHintWrite:
        "Same dimension with a new price: the old row is retired and revision increments; an identical price is a no-op.",
      fieldSiteId: "Site ID",
      fieldProduct: "Product",
      fieldDimKey: "Dimension Key",
      fieldCredits: "Credits",
      fieldCostCents: "Provider Cost (cents)",
      groupImage: "Image (by ratio)",
      groupVideo: "Video (by ratio)",
      groupVideoDuration: "Video (by duration, optional)",
      groupExtend: "Character Extend",
      groupStyle: "Style Rates (by style)",
      groupOther: "Other",
      batchRatio: "Ratio",
      batchFrom: "From (s)",
      batchTo: "To (s)",
      batchCredits: "Credits per entry",
      batchHint:
        "Writes one i2v:<ratio>:<seconds> row per second; existing seconds are repriced (revision+1).",
      batchRun: "Generate",
      batchDone: "Wrote {n} rows",
      writeOk: "Saved",
      colFamily: "Family",
      colEngine: "Engine",
      colReason: "Note",
      colUpdatedAt: "Updated",
      kindAll: "All kinds",
      kindImage: "Image",
      kindVideo: "Video",
      kindCloud: "Cloud",
      filterType: "Type",
      typeAll: "All types",
      filterFamily: "Family / workflow",
      familyAll: "All families",
      filterEngine: "Engine",
      engineAll: "All engines",
      colCover: "Icon",
      overrideYes: "Rewritten",
      btnDisable: "Disable",
      btnEnable: "Enable",
      btnEditMeta: "Edit info",
      disableReason: "Reason",
      disableReasonRequired: "A reason is required to disable",
      switchOk: "Updated",
      batchSelected: "{n} selected",
      batchScope: "Batch scope",
      batchScopeSelected: "Selected rows",
      batchScopeFiltered: "All matching the filter",
      batchEnable: "Enable batch",
      batchDisable: "Disable batch",
      batchEnableConfirm:
        "Enable {n} models? They become visible on the frontend immediately.",
      batchDisableTitle: "Batch disable",
      batchDisableReason: "Batch disable reason",
      batchFilteredHint:
        "Filtering batch affects every model matching the current filter, including rows on other pages.",
      batchNeedCond:
        "A filtered batch needs at least one condition: kind / type / family / engine / keyword",
      batchNeedScope: "Select the models to operate on first",
      batchResult: "Updated {n}",
      batchResultSkipped:
        "Updated {n}, skipped {m} (not in the manageable list)",
      metaTitle: "Edit model info",
      metaHint:
        "Leave blank to use the static definition. Display only: model ID, workflow and pricing are untouched.",
      metaFieldTitle: "Display name",
      metaFieldSummary: "One-line summary",
      metaFieldCover: "Icon / cover URL",
      coverUpload: "Upload icon",
      coverEmpty: "No icon",
      coverTip: "Square PNG/JPG, 256×256 or larger",
      coverUrlHint: "Or paste an image URL",
      metaDefault: "Default: {v}",
      metaEmpty: "(none)",
      metaSaved: "Model info updated",
      metaReset: "Restore default",
      metaResetConfirm:
        "Clear the display info override for this model and restore the static definition?",
      metaResetOk: "Display info restored",
      voMystic: "Adult LoRA (MysticXXX MMH3-V4)",
      voMysticHint:
        "The second LoRA in the reference workflow. On by default for LoRA A/B testing; when off the node is not emitted.",
      voBaseLora: "Base speed LoRA",
      voNsfwLora: "Adult LoRA",
      voSteps: "Steps",
      voShiftVideo: "shift_video",
      voShiftAudio: "shift_audio",
      voSecondsRange: "Duration range (s)",
      voSave: "Save",
      voSaved: "Saved",
      voConfirm:
        "Change the adult-LoRA switch? It affects every video task created afterwards.",
      btnImportLora: "Import from ComfyUI",
      importConfirm:
        "Pull every LoRA from the site ComfyUI into the catalog and auto-mark them via the NSFW blacklist. Continue?",
      colFileName: "File Name",
      colSafety: "Safety",
      safetyAll: "All",
      safetySafe: "Safe",
      safetyAdult: "Adult",
      btnMarkSafe: "Mark Safe",
      btnMarkAdult: "Mark Adult",
      importDone:
        "Import done: total {total}, imported {imported}, updated {updated}, skipped {skipped}, marked adult {adult}",
      colLastSeen: "Last Seen",
      colHolds: "Holds",
      colAvailable: "Available",
      colBalanceCents: "Balance (cents)",
      btnDetail: "Detail",
      btnAdjust: "Adjust",
      btnSuspend: "Suspend",
      btnRestore: "Restore",
      adjustTitle: "Adjust Credit / Balance",
      adjustAsset: "Asset",
      adjustAmount: "Amount",
      adjustReason: "Reason",
      assetCredit: "Credits",
      assetBalance: "Balance (cents)",
      adjustOk: "Adjusted",
      userLedgerTitle: "Wallet Ledger",
      colAsset: "Asset",
      btnCreateTag: "New Tag",
      tagDialogTitle: "Tag",
      colAction: "Actions",
      colAliases: "Aliases",
      colActive: "Active",
      colPriority: "Priority",
      colPosts: "Posts",
      colWorks: "Works",
      fieldTagName: "Name",
      fieldTagAliases: "Aliases (comma separated)",
      fieldTagPriority: "Priority",
      fieldTagActive: "Active",
      saveOk: "Saved",
      // ---- Prompt retention & curation ----
      promptsTitle: "Prompt Library",
      promptsSubtitle: "Inspect prompt logs, curate favorites and promote them to targets",
      tabLogs: "Prompt Logs",
      tabFavorites: "Favorites",
      statTotal: "Total",
      statToday: "Today",
      statUsers: "Users",
      statFavorites: "Favorites",
      statPromoted: "Promoted",
      statTopTools: "Top Tools",
      filterTool: "Tool",
      filterTemplate: "Template",
      filterTimeRange: "Time Range",
      filterFavoritedOnly: "Favorited only",
      filterPromoted: "Promotion",
      promotedAll: "All",
      promotedNone: "Not promoted",
      colPrompt: "Prompt",
      colFinalPrompt: "Final Prompt",
      colTool: "Tool",
      colReuseCount: "Reuse",
      colRating: "Rating",
      colFavoriteId: "Favorite",
      colSessionId: "Session ID",
      colPromptParams: "Params",
      colSource: "Source",
      colTitle: "Title",
      colNegativePrompt: "Negative",
      colTags: "Tags",
      colNote: "Note",
      colPromotedTo: "Target",
      colPromotedRef: "Ref",
      colPromotedAt: "Promoted At",
      colPromptFull: "Full Prompt",
      btnExport: "Export CSV",
      btnFavorite: "Favorite",
      btnEditFavorite: "Edit",
      btnPromote: "Promote",
      btnRemove: "Remove",
      favoriteDialogTitle: "Favorite / Edit",
      detailDialogTitle: "Prompt Detail",
      promoteDialogTitle: "Promote",
      fieldTitle: "Title",
      fieldPrompt: "Prompt",
      fieldNegativePrompt: "Negative",
      fieldTool: "Tool",
      fieldTemplate: "Template",
      fieldTags: "Tags (comma separated)",
      fieldRating: "Rating",
      fieldNote: "Note",
      fieldTarget: "Target",
      ratingHint: "Rating (0-5)",
      confirmRemoveFavorite: "Remove this item from favorites?",
      promptEmpty: "(empty)",
      favoriteSaved: "Favorite saved",
      favoriteRemoved: "Removed from favorites",
      promotedOk: "Promoted",
      promotedAlready: "Promoted",
      exportOk: "Export done",
      exportEmpty: "No data to export",
      noFinalPrompt: "No final prompt",
      fieldRequired: "Please fill in {label}",
    },
  },

  // ==================== Demo Pages ====================
  fun: {
    clipboard: "Clipboard Demo",
    attrClipboard:
      "Thanks to `vue-clipboard3`, https://github.com/JamieCurnow/vue-clipboard3",
    countup: "CountUp Demo",
    attrCountup:
      "Thanks to `countup.js`, https://github.com/inorganik/countUp.js",
    cropper: "Cropper Image Crop",
    attrCropper:
      "Thanks to `cropperjs`, https://github.com/fengyuanchen/cropperjs",
    gridLayout: "Grid Layout Demo",
    attrGridLayout:
      "Thanks to `vue-grid-layout`, https://github.com/jbaysolutions/vue-grid-layout",
    printJs: "Print Demo",
    attrPrintJs:
      "Thanks to `print-js`, https://github.com/crabbly/Print.js. Enable `Background graphics` in print dialog.",
    qrcode: "QRCode Generation",
    attrQrcode:
      "Thanks to `qrcodejs2`, https://github.com/davidshimjs/qrcodejs",
    splitpanes: "Splitpanes Pane Splitter",
    attrSplitpanes:
      "Thanks to `splitpanes`, https://github.com/antoniandre/splitpanes",
    tagsViewDemo: "TagsView Demo",
    tagsViewNotice:
      "The non-current page tagsView demo has been removed. TagsView supports multi-tab (different params) and single-tab sharing (different params).",
    btnRegenerate: "Regenerate",
    btnChangeAvatar: "Change Avatar",
    btnPrintDemo: "Print Demo",
    btnRefreshNum: "Reset / Refresh",
    btnRefreshPage: "Refresh Page",
    btnClosePage: "Close Page",
    countVisitors: "Visitors Today",
    countPresent: "Present",
    countLabTotal: "Lab Total",
    countInUse: "In Use",
    countApplicants: "Applicants (Mo)",
    countApproved: "Approved",
    countSales: "Sales",
    countSalesNum: "Sales Count",

    proDemo: {
      title: "Pro Components Preview",
      subtitle:
        "ProPage + ProSearch + ProToolbar + ProTable + ProDescriptions + ProDrawer + ProUpload with real data",
      badge: "Demo",
      btnDocs: "Docs",
      docsTip: "See the comments in src/components/pro/",
      btnAdd: "Add User",
      btnBatchDelete: "Delete selected",
      colAccount: "Account",
      colNickname: "Nickname",
      colDept: "Dept",
      colRoles: "Roles",
      colMobile: "Mobile",
      btnResetPwd: "Reset Password",
      notSet: "Not set",
      descTitle: "Selected User Details (ProDescriptions demo)",
      descSubtitle: "Schema-driven display with type/render/span/options/nested paths",
      btnClear: "Clear",
      btnOpenDrawer: "Open Drawer (ProDrawer demo)",
      drawerTitle: "User Detail Drawer",
      drawerSubtitle: "ProDescriptions embedded in drawer",
      drawerEmpty: "Click Detail in the table above to select a row",
      confirmDemo: "Confirmed (demo)",
      uploadTitle: "ProUpload Demo",
      uploadSubtitle: "Supports file / image / image-card / drag modes",
      uploadFileMode: "File mode",
      uploadImageMode: "Image mode",
      uploadCardMode: "Image-card mode",
      uploadDragMode: "Drag mode",
      uploadBtnFile: "Upload File",
      uploadTipFile: "Supports doc/pdf/zip, max 20MB",
      uploadBtnImage: "Upload Image",
      uploadTipImage: "Supports JPG / PNG / GIF",
      uploadTipDrag: "Multiple files supported",
      searchKeyword: "Keyword",
      searchKeywordPh: "Account / nickname",
    },
  },

  // ==================== Home Dashboard ====================
  home: {
    cacheDashboard: "Cache Dashboard",
    cacheMode: "Cache Mode",
    cacheKeyCount: "Cache Keys",
    redisConn: "Redis Connection",
    memDistMB: "Memory Distribution (MB)",
    resUsage: "Resource Usage",
    diskUsage: "Disk Usage",
    cpuUsage: "CPU Usage",
    memUsage: "Memory Usage",
    goMemUsage: "Go Memory",
    goMemUsageRate: "Go Memory Rate",
    sysRuntime: "System Runtime",
    usedMem: "Used Memory",
    freeMem: "Free Memory",
    goUsed: "Go Occupied",
    serverName: "Server Name",
    serverIp: "Server IP",
    osName: "OS Name",
    osArch: "OS Arch",
    goVersion: "Go Version",
    cpuCores: "CPU Cores",
    runDuration: "Runtime",
    cpuLoad5min: "CPU Load 5min",
    totalMem: "Total Memory",
    cpuLoad5: "Load5: ",
    coreUnit: " cores",
    dayUnit: "d ",
    hourUnit: "h ",
    minuteUnit: "min",
  },

  // ==================== Common Extensions ====================
  male: "Male",
  female: "Female",

  // ==================== MinIO File Manager ====================
  minio: {
    bucket: "Bucket",
    noBuckets: "No buckets",
    dept: "Dept",
    bucketSettings: "Bucket Settings",
    versioning: "Versioning",
    lifecycle: "Lifecycle",
    lifecycleRules: "Lifecycle Rules",
    encryption: "Encryption",
    bucketPolicy: "Bucket Policy",
    versioningDesc:
      "Keep multiple versions of objects in the same bucket to prevent accidental deletion and overwrites.",
    enableVersioning: "Enable Versioning",
    lifecycleDesc:
      "Automatically manage object lifecycle, e.g. auto-delete expired files.",
    prefixMatch: "Prefix Match",
    prefixPlaceholder: "e.g. logs/",
    expireDays: "Expiration Days",
    serverSideEncryption: "Server-Side Encryption",
    sseDesc:
      "SSE-S3 uses AES-256 to protect data at rest in the bucket. New uploads are automatically encrypted when enabled.",
    sseDisabled:
      "MinIO server is not configured with KMS. SSE-S3 encryption is unavailable. Configure KES key service on MinIO server to enable.",
    enableSSE: "SSE-S3 AES-256 Encryption",
    encryptFailed: "Failed to set bucket encryption: ",
    currentAlgorithm: "Current Algorithm: ",
    uploadFile: "Upload File",
    newDir: "New Folder",
    rename: "Rename",
    fileDetail: "File Details",
    copyLink: "Copy Link",
    loadMore: "Load More",
    cancelSelection: "Cancel Selection",
    tags: "Tags",
    addTag: "Add Tag",
    tagName: "Tag Name",
    tagValue: "Tag Value",
    dirName: "Folder Name",
    parentPath: "Parent Path",
    currentName: "Current Name",
    newName: "New Name",
    inputNewName: "Enter new name",
    create: "Create",
    preview: "Preview",
    filePreview: "File Preview",
    previewNotSupported: "Preview not supported for this file type",
    directory: "Directory",
    file: "File",
    pleaseEnterDirName: "Enter directory name",
    dirCreated: "Directory created",
    pleaseEnterNewName: "Enter new name",
    renameFailed: "Rename failed",
    saveLifecycleRules: "Save Lifecycle Rules",
    addRule: "Add Rule",
    delete: "Delete",
  },

  // Compatibility keys for the simple i18n audit parser.
  ID: "ID",
  URL: "URL",
  Token: "Token",
  Key: "Key",
  COS: "COS",

  // ==================== Attachment Management ====================
  pms_upload: {
    title: "Attachments",
    subtitle: "Manage all uploaded file resources",
    searchPlaceholder: "Search file name...",
    emptyText: "No attachment data",
    uploadFile: "Upload File",
    failed: "Upload Failed",
    success: "Upload Success",
    local: "Local",
    tencentCos: "Tencent COS",
    filesOk: " file(s) uploaded",
    confirmCount: " file(s)?",
    btnUpload: "Upload",
    fileType: "File Type",
    typeImage: "Image",
    typeDoc: "Document",
    typeVideo: "Video",
    typeAudio: "Audio",
    typeArchive: "Archive",
    storageLocal: "Local",
    storageMinIO: "MinIO",
    storageTencent: "Tencent COS",
    colStorageEngine: "Storage Engine",
    colFilename: "Filename",
    colFileSize: "Size",
    colDimensions: "Width × Height",
    fieldExtension: "Extension",
    colCreateDate: "Created",
    colUrl: "URL",
    colMimeType: "MIME Type",
    colSrcFilename: "Source Filename",
    colStoragePath: "Storage Path",
    colExtension: "Extension",
    colStorage: "Storage",
    colUploadTime: "Upload Time",
    dropzoneHint: "Drop files here",
    orClick: "or click to browse",
    uploadBtn: "Start Upload",
    uploading: "Uploading...",
    detailTitle: "File Detail",
    msgNoPreview: "Non-image file, preview unavailable",
    msgSelectDelete: "Please select files to delete",
    statsTotal: "{count} files total",
    uploadSuccess: "{count} file(s) uploaded successfully",
    uploadPartial: "{success} success, {fail} failed",
    confirmSingleDelete: 'Delete "{name}"?',
    confirmBatchDelete: "Delete {count} file(s)?",
  },

  // ==================== Message Queue ====================
  addon_mq: {
    adapter: "Adapters",
    pending: "Pending",
    consumer: "Consumers",
    healthCheck: "Health Check",
    healthPass: "MQ Service OK",
    healthFail: "MQ Service Down",
    peekMessages: "View Messages",
    messageId: "Message ID",
    payload: "Payload",
    payloadSize: "Size",
    payloadDetail: "Message Detail",
    peekCount: "Preview Count",
    noMessages: "No messages",
    noTopic: "No topic data",
    copyPayload: "Copy Payload",
    copySuccess: "Copied to clipboard",
    formatJson: "Formatted",
    rawText: "Raw",
    createdAt: "Created At",
    clearTopic: "Clear",
    clearConfirm: 'Clear all messages in Topic "{name}"?',
    clearSuccess: "Topic {name} cleared",
    clearFailed: "Clear failed",
    dataError: "Failed to fetch MQ data",
  },

  // ==================== Mail ====================
  addon_mail: {
    breadcrumbHome: "Home",
    breadcrumbModule: "Addon",
    breadcrumbCurrent: "Mail Log",
    listTitle: "Send History",
    filterRecipient: "Recipient",
    placeholderRecipient: "Enter recipient email",
    filterStatus: "Status",
    placeholderStatus: "All",
    filterDriver: "Driver",
    placeholderDriver: "All",
    statusSuccess: "Success",
    statusFail: "Failed",
    colRecipient: "Recipient",
    colSubject: "Subject",
    colDriver: "Driver",
    colStatus: "Status",
    colError: "Error",
    colTime: "Time",
    colDeliver: "Delivery",
    btnResend: "Resend",
    confirmResend: "Resend to {to}?",
    resendSuccess: "Resend success",
    btnSendTest: "Send Test Mail",
    sendTestTitle: "Send Test Mail",
    sendTestTo: "Recipient Email",
    sendTestToPlaceholder: "Enter recipient email",
    sendTestSubject: "Subject",
    sendTestBody: "Body (HTML)",
    sendTestSuccess: "Send request submitted",
    balanceTitle: "SendCloud Account",
    balance: "Balance",
    avaliableBalance: "Available",
    quota: "Daily Quota",
    usedQuota: "Used",
    reputation: "Reputation",
    accountType: "Account Type",
    btnWebhook: "Webhook Config",
    webhookTitle: "SendCloud Webhook Config",
    webhookUrl: "Callback URL",
    webhookUrlPlaceholder: "https://your-domain/api/v1/mail/webhook/sendcloud",
    categoryName: "API_USER (or all)",
    events: "Events (comma separated, empty=all)",
    eventsExample: "e.g. 1,10,11 (1 delivered, 10 click, 11 open)",
    addWebhook: "Add",
    deleteWebhook: "Delete",
    confirmDeleteWebhook: "Delete this webhook config?",
    webhookEmpty:
      "No webhook configured — SendCloud won't push delivery events",
    deliverUnknown: "Unknown",
    deliverDelivered: "Delivered",
    deliverBounce: "Soft Bounce",
    deliverInvalid: "Invalid",
    deliverOpened: "Opened",
    deliverClicked: "Clicked",
    deliverSpam: "Spam",
    deliverUnsub: "Unsubscribed",
    statsTitle: "Last 7 Days Stats",
    statsTotal: "Total Sent",
    statsDelivered: "Delivered",
    statsOpened: "Opened",
    statsClicked: "Clicked",
    statsBounce: "Bounce",
    statsFailed: "Failed",
  },

  addon_sitemap: {
    domain: "Domain",
    push: "Push",
    generate: "Generate Sitemap",
    pushApi: "Push API",
    domainRequired: "Please enter website domain",
    generateDone: "Generated",

    generateFailed: "Generation failed",
    responsePrefix: "Response prefix",
    pushFailed: "Push failed",
  },

  // ==================== Short Link ====================
  addon_sitelink: {
    title: "Site Links",
    breadcrumbHome: "Home",
    breadcrumbModule: "Addons",
    subtitle: "Cross-site traffic: link sites to each other (nofollow by default)",
    colSourceSite: "Source Site",
    colTargetSite: "Target Site",
    colAnchor: "Anchor Text",
    colTargetUrl: "Target URL",
    colPosition: "Placement",
    colSort: "Sort",
    allSites: "All sites",
    statusOn: "Enabled",
    statusOff: "Disabled",
    btnAdd: "Add Link",
    dialogAdd: "Add Link",
    dialogEdit: "Edit Link",
    phAnchor: "Link text, e.g. Recommended sites",
    phTargetUrl: "/channel/path or an absolute URL",
    ruleTargetSite: "Target site is required",
    ruleAnchor: "Anchor text is required",
    ruleTargetUrl: "Target URL is required",
    rulePosition: "Placement is required",
    delConfirm: "Delete link {anchor}?",
  },

  addon_shortlink: {
    title: "Short Links",
    plugin: "Addons",
    subtitle:
      "Site-scoped short links: code → target URL with 302/301 redirect; main CTA, expiry and click stats (data scoped by the site selected in the top-right)",
    add: "Add Short Link",
    addTitle: "Add Short Link",
    editTitle: "Edit Short Link",
    keywordPlaceholder: "Search code / name",
    allStatus: "All statuses",
    enabled: "Enabled",
    disabled: "Disabled",
    search: "Search",
    reset: "Reset",
    code: "Code",
    name: "Name",
    targetUrl: "Target URL",
    status: "Status",
    redirectType: "Redirect",
    mainCta: "Main CTA",
    mainCtaTag: "Main CTA",
    setDefault: "Set default",
    setDefaultConfirm: "set as the current site main CTA",
    cancelCtaConfirm: "cancel the main CTA",
    setDefaultAsk: 'Confirm {action} for "{code}"?',
    setDefaultOk: "Set as main CTA",
    cancelCtaOk: "Cancelled",
    expireAt: "Expires at",
    expirePlaceholder: "Empty = never expires",
    clickCount: "Clicks",
    createdAt: "Created at",
    actions: "Actions",
    edit: "Edit",
    stats: "Stats",
    statsTitle: "Click Stats",
    del: "Delete",
    delConfirm: 'Delete short link "{code}"?',
    delOk: "Deleted",
    tip: "Notice",
    save: "Save",
    saveOk: "Saved",
    cancel: "Cancel",
    random: "Random",
    codePlaceholder: "2-16 alphanumeric; leave empty to auto-generate",
    codeRuleMsg: "2-16 alphanumeric characters",
    namePlaceholder: "e.g. Homepage CTA",
    urlPlaceholder: "https://example.com/page (http/https only)",
    urlRequired: "Please enter target URL",
    urlRuleMsg: "http/https links only",
    redirect302: "302 temporary redirect",
    redirect301: "301 permanent redirect",
    enableOk: "Enabled",
    disableOk: "Disabled",
    copyOk: "Copied: {url}",
    copyFail: "Copy failed, please copy manually",
    totalClicks: "Total clicks",
    days7: "7d",
    days14: "14d",
    days30: "30d",
    noData: "No click data yet",
  },

  // ==================== Short.io Links ====================
  addon_shortio: {
    title: "Short.io Links",
    plugin: "Addons",
    subtitle:
      "Short.io-hosted short link management: code → target URL redirect with expiry and click stats (account-level, not site-scoped)",
    add: "Add Short Link",
    addTitle: "Add Short Link",
    editTitle: "Edit Short Link",
    keywordPlaceholder: "Search code / title",
    allStatus: "All statuses",
    enabled: "Enabled",
    disabled: "Disabled",
    search: "Search",
    reset: "Reset",
    code: "Code",
    name: "Title",
    targetUrl: "Target URL",
    status: "Status",
    redirectType: "Redirect",
    redirectDefault: "Domain default",
    expireAt: "Expires at",
    expirePlaceholder: "Empty = never expires",
    clickCount: "Clicks",
    createdAt: "Created at",
    actions: "Actions",
    edit: "Edit",
    stats: "Stats",
    statsTitle: "Click Stats",
    setCta: "Set CTA",
    setCtaConfirm:
      'Set "{code}" as the current site main CTA?\n{url}\n(All CTA buttons on the site point to this link immediately, no template change needed)',
    setCtaOk: "Set as the current site main CTA",
    del: "Delete",
    delConfirm: 'Delete short link "{code}"?',
    delOk: "Deleted",
    tip: "Notice",
    save: "Save",
    saveOk: "Saved",
    cancel: "Cancel",
    codePlaceholder: "Leave empty to auto-generate (1-64 alphanumeric/_-)",
    codeRuleMsg: "1-64 alphanumeric, underscore or hyphen",
    namePlaceholder: "e.g. Homepage CTA",
    urlPlaceholder: "https://example.com/page (http/https only)",
    urlRequired: "Please enter target URL",
    urlRuleMsg: "http/https links only",
    enableConfirm: 'Enable short link "{code}"?',
    disableConfirm: 'Disable short link "{code}"?',
    enableOk: "Enabled",
    disableOk: "Disabled",
    archivedHint:
      "Archived links no longer appear in the list (restore them in the short.io dashboard)",
    copyOk: "Copied: {url}",
    copyFail: "Copy failed, please copy manually",
    totalClicks: "Total clicks",
    days7: "7d",
    days14: "14d",
    days30: "30d",
    noData: "No click data yet",
  },

  // ==================== TAG Management ====================
  tpl: {
    searchPlaceholder: "Search functions…",
    stats: {
      categories: "categories",
      funcs: "functions",
    },
    noMatch: "No matching functions",
    emptyHint: "Press / to focus search",
    output: "{engine} output",
    copied: "Copied",
    action: {
      copy: "Copy Code",
      enableLoop: "Enable Loop",
    },
    form: {
      loopVar: "Loop Var",
      itemFields: "Item Fields",
    },
    tips: {
      selectFunction: "Select a function from the left panel",
    },
    toggle: {
      dynamicChannel: "Dynamic Channel ID",
    },
    hint: {
      dynamicChannel:
        'Ignore channel_id param, use printf "channel_id:%d|..." .Cms_Channel.Id to get from current page',
    },
    catalog: {
      i18n: "i18n Translation",
      query: "Data Query",
      url: "URL Generation",
      format: "Formatting",
      pager: "Pagination",
      global: "Global Functions",
      item: "Item Methods",
    },
    desc: {
      catalog: {
        i18n: "Multi-language translation and language info",
        query: "Articles/Channels/Tags/Blocks/Pages query",
        url: "Article/Tag/Channel/Home URL generation",
        format: "Date/Text/Number formatting",
        pager: "Pagination info builder",
        global: "Utility functions (Default/CorrectionPath)",
        item: "Article item methods (used inside loops)",
      },
      L: "Translate a key by current language",
      Articles: "Query article list with multi-language filter",
      Menu: "Tree structure channel menu with language prefix",
      Breadcrumb: "Home → Parent chain → Current channel",
      Adjacent: "Previous and next article",
      Pager: "Build pagination info (PageInfo)",
      Default: "Return first non-empty value",
      CorrectionPath: "Auto-prefix / for relative paths",
    },
    func: {
      L: "Translate Text",
      Ld: "Translate with Default",
      Lang: "Current Language",
      Langs: "Language List",
      Articles: "Article List",
      Items: "Default Article List",
      Tags: "Tag List",
      Menu: "Channel Menu",
      NavMenus: "Flat Nav",
      Block: "Block Content",
      Channel: "Channel Info",
      Breadcrumb: "Breadcrumb",
      Adjacent: "Prev/Next",
      Page: "Page Content",
      Config: "Site Config",
      Home: "Home URL",
      Url: "Article URL",
      TagUrl: "Tag URL",
      ChanUrl: "Channel URL",
      Date: "Date Format",
      Cut: "Text Truncate",
      Num: "Number Format",
      Now: "Current Time",
      Pager: "Pagination",
      Paginate: "Pagination (alias)",
      Default: "Default Value",
      CorrectionPath: "Path Fix",
      Id: "Article ID",
      Title: "Title",
      Desc: "Description",
      HTML: "HTML Body",
      Text: "Templ Body",
      Pic: "Thumbnail URL",
      HasPic: "Has Thumbnail",
      Views: "Page Views",
      Flag: "Flag Field",
      HasFlag: "Has Flag",
      FlagClass: "CSS Class",
      ChannelId: "Channel ID",
      TagList: "Tag Array",
      FlagBits: "Flag Bits",
      IsTop: "Is Top",
      Recommended: "Is Recommended",
      PublishedAt: "Published At",
      Url0: "Article URL (no param)",
      Module: "Module Field",
      ModuleMap: "Module Field Map",
    },
    param: {
      title: "Parameters",
      channel_id: "Channel ID",
      model_id: "Model ID",
      status: "Status",
      row: "Page Size",
      page: "Page",
      order: "Sort Field",
      sort: "Sort Order",
      flag: "Flag",
      keywords: "Keywords",
      is_hot: "Hot",
      is_new: "New",
      is_recommend: "Recommended",
      related_id: "Related ID",
      parent_id: "Parent ID",
      activeId: "Active ID",
      key: "Key",
      def: "Default",
      id: "ID",
      name: "Name",
      diyname: "Diyname",
      tag: "Tag",
      diy: "Diyname",
      val: "Value",
      format: "Format",
      s: "Text",
      n: "Length",
      v: "Number",
      total: "Total",
      module_key: "Module Field Name",
    },

    // ===== Hardcoded Chinese key fallback translations =====
    "0=全平台": "0=All Platforms",
    "App Store 退款": "App Store Refund",
    "CMS 栏目管理 · 树形结构": "CMS Channel · Tree View",
    "CMS 配置分组 · 自定义字段": "CMS Config Groups · Custom Fields",
    "CP 订单号": "CP Order No.",
    "Google Play 退款": "Google Play Refund",
    "IP 地址": "IP Address",
    "MinIO 服务端未配置 KMS，SSE-S3 加密不可用。如需启用，请在 MinIO 服务端配置 KES 密钥服务。":
      "KMS not configured; SSE-S3 encryption unavailable. Set up KES key service to enable.",
    "PMS 模型信息管理": "PMS Model Info Management",
    "PMS 部门管理": "PMS Dept Management",
    "SEO 标题": "SEO Title",
    "SSE-S3 AES-256 加密": "SSE-S3 AES-256 Encryption",
    "SSE-S3 使用 AES-256 保护存储桶中的静态数据。开启后新上传文件自动加密。":
      "SSE-S3 protects data at rest with AES-256. New uploads are encrypted automatically.",
    "elementPlus 图标": "ElementPlus Icons",
    "fontawesome 图标": "FontAwesome Icons",
    "iconfont 图标": "Iconfont Icons",
    "• 结束时间为空 = 永久封禁": "• Empty end time = permanent ban",
    上传中: "Uploading",
    "上传中...": "Uploading...",
    上传图片: "Upload Image",
    上传文件: "Upload File",
    上传时间: "Upload Time",
    下单开始: "Order Start",
    下单结束: "Order End",
    交易ID: "Transaction ID",
    付费统计: "Payment Stats",
    代码: "Code",
    保存修改: "Save Changes",
    保存生命周期规则: "Save Lifecycle Rule",
    "允许在同一个存储桶中保留对象的多个版本，防止意外删除和覆盖。":
      "Keep multiple object versions in one bucket to prevent accidental deletion.",
    "免费注册获取 API Key：": "Free registration for API Key:",
    "免费版每月 250 次请求 | API 文档：":
      "Free tier: 250 req/month | API docs:",
    公司或办公地址: "Company/Office Address",
    公开读: "Public Read",
    内容模板: "Content Template",
    分: "min",
    "创建时间:": "Created:",
    删除此翻译: "Delete This Translation",
    前缀匹配: "Prefix Match",
    加密: "Encrypt",
    "加载区块信息...": "Loading block info...",
    "加载单页信息...": "Loading page info...",
    "加载开发商信息...": "Loading developer info...",
    加载更多: "Load More",
    "加载标签信息...": "Loading tag info...",
    "加载栏目信息...": "Loading channel info...",
    "加载模型信息...": "Loading model info...",
    "加载用户详情...": "Loading user details...",
    "加载订单详情...": "Loading order details...",
    包名: "Package Name",
    区块管理: "Block Management",
    压缩包: "Archive",
    原始文件名: "Original Filename",
    发布设置: "Publish Settings",
    发货: "Fulfill",
    取消选择: "Deselect",
    可翻译: "Translatable",
    启用后该字段在多语言编辑时可填入翻译值:
      "Enable to allow translation input for this field.",
    启用版本控制: "Enable Versioning",
    响应内容: "Response Content",
    商品: "Product",
    "商品 ID": "Product ID",
    商品ID: "Product ID",
    固定汇率: "Fixed Rate",
    图片: "Image",
    地区代码: "Region Code",
    地区名称: "Region Name",
    基本信息: "Basic Info",
    复制链接: "Copy Link",
    大小: "Size",
    天: "d",
    头像: "Avatar",
    "如 http://*.example.com/*": "e.g. http://*.example.com/*",
    "如 images": "e.g. images",
    "如 logs/": "e.g. logs/",
    存储: "Storage",
    存储引擎: "Storage Engine",
    存储桶: "Bucket",
    完成: "Done",
    密码: "Password",
    "导出 Excel": "Export Excel",
    封禁: "Ban",
    封禁信息: "Ban Info",
    封禁原因: "Ban Reason",
    封禁目标: "Ban Target",
    封禁类型: "Ban Type",
    封禁说明: "Ban Description",
    小时: "h",
    "尚未配置 API Key，请粘贴 apilayer API Key":
      "API Key not configured. Paste your apilayer API Key.",
    尺寸: "Dimensions",
    "已创建 → 已提交 → 已完成 / 失败。":
      "Created → Submitted → Completed / Failed.",
    已过期: "Expired",
    "已配置 API Key:": "API Key configured:",
    平台: "Platform",
    应用: "App",
    "开发商 ID:": "Developer ID:",
    开发商名称: "Developer Name",
    开始上传: "Start Upload",
    当前名称: "Current Name",
    当前状态: "Current Status",
    "当前算法：": "Current Algorithm:",
    微信号: "WeChat ID",
    总充值金额: "Total Recharge",
    或点击下方按钮选择: "Or click the button below to select",
    "或点击选择文件，支持多文件上传":
      "Or click to select files (multi-file supported)",
    所属模块: "Module",
    扩展名: "Extension",
    批量删除: "Batch Delete",
    拖拽文件到此处: "Drag files here",
    拖拽文件或文件夹到此处: "Drag files or folders here",
    "控制桶的访问权限。设为公开可匿名读取，私有需预签名 URL，防盗链仅允许白名单域名。":
      "Control bucket access: Public = anonymous read, Private = presigned URL, Hotlink = whitelist only.",
    "提交中...": "Submitting...",
    "搜索文件名...": "Search filename...",
    "搜索用户ID / 用户名 / 邮箱...": "Search by user ID / username / email...",
    操作日志详情: "Operation Log Details",
    支付: "Payment",
    支付防重去重记录: "Payment Dedup Record",
    文件名: "Filename",
    文件类型: "File Type",
    文件详情: "File Details",
    文档: "Document",
    "文章以默认语言创建，多语言翻译请在编辑页面添加":
      "Article created in default language. Add translations on the edit page.",
    新名称: "New Name",
    新建区块: "Create Block",
    新建单页: "Create Page",
    新建标签: "Create Tag",
    新建栏目: "Create Channel",
    新建模型信息: "Create Model Info",
    新建目录: "Create Directory",
    无法加载: "Failed to Load",
    无法加载区块信息: "Failed to load block info",
    无法加载单页信息: "Failed to load page info",
    无法加载开发商信息: "Failed to load developer info",
    无法加载标签信息: "Failed to load tag info",
    无法加载栏目信息: "Failed to load channel info",
    无法加载模型信息: "Failed to load model info",
    无法加载订单: "Failed to load order",
    时间: "Time",
    时间信息: "Time Info",
    昵称: "Nickname",
    暂无充值记录: "No recharge records",
    暂无存储桶: "No buckets",
    暂无用户数据: "No user data",
    暂无订单数据: "No order data",
    暂无配置数据: "No config data",
    暂无附件数据: "No attachment data",
    更多条件: "More Filters",
    "更新时间:": "Updated:",
    最后登录: "Last Login",
    "最后登录 IP": "Last Login IP",
    "服务器 ID": "Server ID",
    服务器ID: "Server ID",
    服务端加密: "Server-side Encryption",
    本地: "Local",
    机型: "Device Model",
    查单: "Check Order",
    查看与管理所有支付订单: "View & manage all payment orders",
    标志: "Flag",
    标签: "Tag",
    标签值: "Tag Value",
    标签名: "Tag Name",
    栏目不存在或已被删除: "Channel does not exist or has been deleted",
    栏目管理: "Channel Management",
    桶策略: "Bucket Policy",
    桶设置: "Bucket Settings",
    桶访问策略: "Bucket Access Policy",
    模块绑定: "Module Binding",
    模型字段: "Model Fields",
    永久: "Permanent",
    "汇率API Key 配置": "Exchange Rate API Key Config",
    注册开始: "Registration Start",
    注册时间: "Registration Time",
    注册结束: "Registration End",
    添加域名: "Add Domain",
    添加文章: "Add Article",
    添加标签: "Add Tag",
    "添加用户/设备/IP 封禁记录": "Add user/device/IP ban record",
    添加规则: "Add Rule",
    清空: "Clear",
    游客: "Guest",
    "游戏 AppID": "Game AppID",
    游戏名称: "Game Name",
    游戏昵称: "Game Nickname",
    游戏范围: "Game Scope",
    游戏角色: "Game Character",
    点击上传: "Click to Upload",
    "点击图片切换到下一张 · 键盘 ← → 导航":
      "Click image to switch, keyboard ← → to navigate",
    父路径: "Parent Path",
    版本控制: "Version Control",
    "状态说明：": "Status:",
    生命周期: "Lifecycle",
    生命周期规则: "Lifecycle Rule",
    生效中: "Active",
    用户: "User",
    "用户 ID": "User ID",
    "用户 UID": "User UID",
    "用户-游戏关联列表": "User-Game Association List",
    用户与角色: "Users & Roles",
    用户名: "Username",
    "留空=永久封禁": "Leave empty = permanent ban",
    登录: "Login",
    登录历史: "Login History",
    登录次数: "Login Count",
    目录名称: "Directory Name",
    确认添加: "Confirm Add",
    社交账号: "Social Accounts",
    私有: "Private",
    秒: "s",
    移除: "Remove",
    符号: "Symbol",
    等待中: "Pending",
    "策略 JSON": "Policy JSON",
    "管理所有 SDK 注册用户与登录记录":
      "Manage all SDK registered users & login records",
    管理所有上传的文件资源: "Manage all uploaded file resources",
    "粘贴你的 apilayer API Key": "Paste your apilayer API Key",
    系统: "System",
    绑定时间: "Bind Time",
    编辑文章: "Edit Article",
    编辑通道配置: "Edit Channel Config",
    网站域名: "Domain Name",
    联系人: "Contact",
    联系人姓名: "Contact Name",
    联系地址: "Contact Address",
    联系方式: "Contact Method",
    联系电话: "Contact Phone",
    联系邮箱: "Contact Email",
    腾讯COS: "Tencent COS",
    "自动管理对象的生命周期，例如自动删除过期文件。":
      "Automatically manage object lifecycle, e.g. auto-delete expired files.",
    补单: "Supplement Order",
    视频: "Video",
    解封: "Unban",
    "订单 ID": "Order ID",
    订单ID: "Order ID",
    订单信息: "Order Info",
    订单数量: "Order Count",
    订单状态: "Order Status",
    设备: "Device",
    "设备 ID": "Device ID",
    设备信息: "Device Info",
    设备号: "Device ID",
    设备记录: "Device Records",
    "访问 URL": "Access URL",
    该文件类型暂不支持预览: "Preview not supported for this file type",
    请传入模型ID或启用模型选择器:
      "Please provide a Model ID or enable the model selector",
    账号: "Account",
    账号类型: "Account Type",
    购买Token: "Buy Token",
    输入新名称: "Enter New Name",
    过期天数: "Expiry Days",
    近一月充值记录: "Recharge Records (Last Month)",
    返回游戏列表: "Back to Game List",
    进度: "Progress",
    退款日期: "Refund Date",
    退款次数: "Refund Count",
    选择开始时间: "Select Start Time",
    选择文件: "Select File",
    选择文件夹: "Select Folder",
    "通知 URL": "Notify URL",
    通知URL: "Notify URL",
    "通知内容 (JSON)": "Notify Content (JSON)",
    "通知状态：": "Notify Status:",
    通道编码: "Channel Code",
    通道配置: "Channel Config",
    邮箱: "Email",
    重命名: "Rename",
    "重新通知。": "Re-notify.",
    重置密码: "Reset Password",
    金额: "Amount",
    防盗链: "Hotlink Protection",
    防盗链域名白名单: "Hotlink Domain Whitelist",
    "防盗链模式允许空 Referer（地址栏直接访问）和白名单域名。示例：":
      "Hotlink mode allows empty Referer (direct access) and whitelisted domains. Example:",
    "非图片文件，无预览": "No preview for non-image files",
    音频: "Audio",
    高级搜索: "Advanced Search",
    默认: "Default",
    默认语言: "Default Language",
    "（游戏服务器未正确响应）。通知失败时可点击":
      "(Game server did not respond). Click on notify failure",
    "（游戏服务器返回 SUCCESS）/": "(Game server returned SUCCESS) /",
  },

  // ==================== Backup / 备份 ====================
  backup: {
    home: "Home",
    dataBackup: "Data Backup",
    fileList: "Backup File List",
    selectDriver: "Select Backup Method",
    createBackup: "Create Backup",
    fileName: "File Name",
    size: "Size",
    createTime: "Created At",
    action: "Actions",
    download: "Download",
    delete: "Delete",
    pleaseSelectDriver: "Please select a backup method",
    backupCreated: "Backup created: ",
    backupCreateFailed: "Backup creation failed",
    confirmDeleteBackup: "Confirm delete backup",
    confirm: "Confirm",
    deleted: "Deleted",
  },
  // ==================== Comment / 评论 ====================
  comment: {
    commentManage: "Comment Management",
    all: "All",
    pending: "Pending",
    approved: "Approved",
    rejected: "Rejected",
    recycleBin: "Recycle Bin",
    searchContent: "Search comment content",
    search: "Search",
    batchApprove: "Batch Approve",
    batchReject: "Batch Reject",
    moveToRecycle: "Move to Recycle",
    nickname: "Nickname",
    content: "Content",
    articleId: "Article ID",
    time: "Time",
    approvedLabel: "Approved",
    rejectedLabel: "Rejected",
    restore: "Restore",
    destroy: "Permanently Delete",
    movedToRecycle: "Moved to recycle bin",
    batchedRejected: "Batched rejected",
    unknown: "Unknown",
    approve: "Approve",
    restored: "Restored",
    destroyed: "Destroyed",
    batchedApproved: "Batch approval completed",

    status: "Status",
    action: "Actions",
    delete: "Delete",
  },
  // ==================== Form / 表单 ====================
  form: {
    exportCsv: "Export CSV",
    deleteSelected: "Delete Selected",
    confirmDeleteData: "Confirm delete selected data?",
    addField: "Add Field",
    saveAllFields: "Save All Fields",
    fieldLabel: "Field Label",
    text: "Text",
    textarea: "Textarea",
    select: "Select",
    radio: "Radio",
    checkbox: "Checkbox",
    file: "File",
    email: "Email",
    date: "Date",
    number: "Number",
    required: "Required",
    placeholder: "Placeholder",
    optionsHint: "Options (comma separated)",
    noFields: 'No fields. Click "Add Field"',
    fieldSaved: "Fields saved",
    formManage: "Form Management",
    newForm: "New Form",
    formTitle: "Form Title",
    fieldCount: "Fields",
    submitCount: "Submissions",
    fieldBuilder: "Field Builder",
    data: "Data",
    editForm: "Edit Form",
    submitBtn: "Submit Button",
    successMsg: "Success Message",
    notifyEmail: "Notification Email",
    commaSep: "Comma separated",
    cancel: "Cancel",
    submit: "Submit",
    submitSuccess: "Submitted",
    pleaseEnterTitle: "Please enter form title",
    saved: "Saved",
    created: "Created",
    confirmDeleteForm: "Deleting will also remove fields and data. Confirm?",
    actionSettings: "Settings",

    time: "Time",
    confirm: "Confirm",
    deleted: "Deleted",
    createTime: "Created",
    action: "Actions",
    delete: "Delete",
    status: "Status",
  },
  // ==================== Webhook ====================
  webhook: {
    addWebhook: "Add Webhook",
    name: "Name",
    targetUrl: "Target URL",
    event: "Event",
    lastStatus: "Last Status",
    lastResponse: "Last Response",
    articlePublished: "Article Published",
    articleUpdated: "Article Updated",
    articleDeleted: "Article Deleted",
    commentCreated: "Comment Created",
    commentApproved: "Comment Approved",
    signSecret: "Signing Secret",
    signSecretHint: "Optional, HMAC signature",
    timeoutSec: "Timeout (s)",
    retryCount: "Retry Count",
    pleaseFillNameUrl: "Please fill in name and URL",
    confirmDelete: "Confirm delete?",
    editWebhook: "Edit Webhook",

    status: "Status",
    createTime: "Created",
    action: "Actions",
    delete: "Delete",
    cancel: "Cancel",
    saved: "Saved",
    created: "Created",
    confirm: "Confirm",
    deleted: "Deleted",
  },
  // ==================== CMS Article / 文章 ====================
  cmsArticle: {
    revisionHistory: "Revision History",
    remark: "Remark",
    operateTime: "Operated At",
    view: "View",
    restore: "Restore",
    fieldSnapshot: "Field Snapshot",
    fieldsCount: "Fields",
    contentSnapshot: "Content Snapshot",
    seoCheck: "SEO Check",
    seoAutoCheck: "Auto-check after entering title",
    saveDraft: "Save Draft",
    unpublished: "Unpublished",
    published: "Published",
    draft: "Draft",
    scheduled: "Scheduled",
    confirmRestore:
      "Restore this version? Current content will be saved as snapshot",
    confirmRestoreTitle: "Confirm Restore",
    restoredToRevision: "Restored to revision",
    batchSuccess: "Batch operation completed",
    moveChannel: "Move Channel",
    setTags: "Set Tags",
    setStatus: "Set Status",
    selectTargetChannel: "Select target channel",
    inputTagsCsv: "Enter tags, comma separated",
    selectStatus: "Select status",
    confirmExecute: "Confirm Execute",

    action: "Actions",
    unknown: "Unknown",
    btnRegenerateStatic: "Regenerate static pages",
    cancel: "Cancel",
    confirmBatchRegen: "Regenerate static pages for selected articles?",
    titleBatchRegen: "Batch regeneration",
    msgRegenerating: "Regenerating, please wait…",
    msgRegenerateDone: "Regeneration complete",
  },
  // ==================== CMS Link / 友链 ====================
  menu_cms_link: "Links",
  menu_cms_link_add: "Add Link",
  menu_cms_link_edit: "Edit Link",
  cmsLink: {
    logoUrl: "Logo URL",
    target: "Target",
    targetBlank: "Open in new tab",
    targetSelf: "Open in current tab",
    group: "Group",
    selectGroup: "Select Group",
    urlRequired: "URL is required",
    added: "Added",
    addSuccess: "Added successfully",
    missingId: "Missing ID",
    saveSuccess: "Saved successfully",
  },
  // ==================== SDK User ====================
  sdkUser: {
    confirmKickOut: "Confirm kick out this device?",
    confirmKickOutTitle: "Confirm Kick Out",
  },

  // ==================== Addon Document / Documents ====================
  document: {
    title: "Documents",
    subtitle:
      "Manage document trees, local Markdown source files, and multilingual publishing",
    category: "Category",
    addCategory: "Add Category",
    editCategory: "Edit Category",
    addDocument: "Add Document",
    editDocument: "Edit Document",
    allCategory: "All Categories",
    allLang: "All Languages",
    allStatus: "All Status",
    rootCategory: "Root Category",
    parentCategory: "Parent Category",
    categoryName: "Category Name",
    sourceCategoryId: "Linked Category",
    sourceCategoryHint:
      "Link to the corresponding category in another language (for language switching)",
    langZhCn: "Chinese (Simplified)",
    langEnUs: "English",
    langUniversal: "Universal (All Languages)",
    weighHint: "Higher number = higher position",
    docTitle: "Document Title",
    description: "Description",
    summary: "Summary",
    markdown: "Markdown",
    markdownHint:
      "Rich text editor with Markdown export. Supports [[slug]] / [[slug|label]] cross references",
    lang: "Language",
    views: "Views",
    published: "Published",
    draft: "Draft",
    keyword: "Keyword",
    searchKeyword: "Search title, slug, summary",
    selectCategory: "Select Category",
    fillRequired: "Please fill required fields",
    saved: "Saved successfully",
    deleted: "Deleted successfully",
    refresh: "Refresh",
    confirmDeleteCategory: "Confirm delete this category?",
    confirmDeleteDocument: "Confirm delete this document?",
    syncSource: "Sync Current Site Document Source",
    confirmSyncSource:
      "Scan resource/document/{siteCode}/cms-manual of the current site and update this site's document indexes from local Markdown sources (other sites are not affected). Continue?",
    syncResult: "Sync Result",
    syncComplete: "Sync completed",
    sourceFile: "Source File",
    sourceFileHint:
      "Leave empty to auto-generate by language and slug, e.g. zh-CN/getting-started/overview.md",
    sourceHint:
      "Body content comes from local Markdown source files. Saving writes back to .md; the database only stores structure and file metadata.",
    sourceHash: "File Hash",
    sourceMtime: "File Modified At",
    syncedAt: "Synced At",
    scanned: "Scanned",
    created: "Created",
    updated: "Updated",
    hidden: "Hidden",
    categories: "Categories",
    errors: "Errors",
  },

  // ==================== Common Components ====================
  component: {
    uploadImage: "Upload image",
    uploading: "Uploading",
    editorPlaceholder: "Enter content...",
    noIcon: "No icons found",
    selectIcon: "Select icon",
    iconSearchPlaceholder: "Search or select an icon",
    fontawesomeIcon: "Font Awesome icons",
    elementPlusIcon: "Element Plus icons",
    iconfontIcon: "iconfont icons",
    uploadImageOnly: "Please upload an image file",
    uploadSizeLimit: "Please upload within {size} MB",
    uploadSizeExceed: "File size cannot exceed {size}MB",
    uploadCountExceed: "Max {count} files allowed. Limit exceeded.",
    confirmDeleteFile: "Are you sure to delete {name}?",
    fieldExtension: "{name} Extended Fields",
    fieldMaxLength: "{label} cannot exceed {max} characters",
    clickToUpload: "Click to Upload",
    dragText: "Drop files here or ",
    noModelHint: "Please provide a Model ID or enable Model Selector",
  },

  // ==================== Layout Extras ====================
  layoutFooterCopyright:
    "Yunnan Qixun Technology Co., Ltd. All Rights Reserved.",
  layoutPrimaryColorRequired: "Primary color is required",
  layoutTagsStyle1: "Style 1",
  layoutTagsStyle4: "Style 4",
  layoutTagsStyle5: "Style 5",
  layoutColumnsRound: "Rounded",
  layoutColumnsCard: "Card",
  layoutColumnsHorizontal: "Horizontal",
  layoutColumnsVertical: "Vertical",

  editSuccess: "Updated",

  addSuccess: "Added",

  weightUpdated: "Weight updated",

  saveSuccess: "Saved",

  createSuccess: "Created",

  deleteSuccess: "Deleted",
};
