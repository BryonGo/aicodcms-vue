...
<script lang="ts">
import { toRefs, reactive, computed, onMounted, onBeforeUnmount, defineComponent, ref } from "vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    const initEchartsMap = () => {
      myChartRef.value = echarts.init(<HTMLElement>state.echartsMap);
      const option = {
        tooltip: {
          trigger: "item",
        },
    ...
    // 页面加载时
    onMounted(() => {
      //initEchartsMap(); (commented out intentionally)
    });
    // 组件卸载时清理 ECharts 实例和事件监听
    onBeforeUnmount(() => {
