...
export interface CmsArticleInfoData {
  ...
  is_jump: string | number | undefined; // 是否跳转地址
  ...
  is_top: string | undefined; // 是否置顶;1:置顶;0:不置顶
  status: number | undefined; // 状态
  weigh: number | undefined; // 权重
  showtpl: string | undefined; // 自定义详情模板
  content: string | undefined; // 文章内容
  modulesData: any; // 模型数据
  is_hot?: string; // 是否热门;1:是;0:否
  is_new?: string; // 是否最新;1:是;0:否
  is_recommend?: string; // 是否推荐;1:是;0:否
  price?: number; // 付费阅读价格
